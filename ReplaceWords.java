...
*/
    ...
    public String replaceWords(List<String> dictionary, String sentence) {
        ...
        for(String str : strArray) {
            String root = checkRootInDictionary(dictionary, str);
            result.append(root + " ");
        }
        return result.toString().trim();
    }

    public String checkRootInDictionary (List<String> dictionary, String str) {
        String result = str;
        for(String root : dictionary) {
            if(str.startsWith(root) && root.length() < result.length())
                result = root;
        }
        return result;
    }
